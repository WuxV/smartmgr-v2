...
class DiskgroupDropMachine(BaseMachine):
    __metaclass__ = MataMachine

    MID = msg_mds.DISKGROUP_DROP_REQUEST

    def INIT(self, request):
        self.response     = MakeResponse(msg_mds.DISKGROUP_DROP_RESPONSE, request)
        self.request      = request
        self.request_body = request.body.Extensions[msg_mds.diskgroup_drop_request]
        ...
        ret, result = sql.drop_asm_group(diskgroup_name)
        if ret:
            self.response.rc.retcode = msg_mds.RC_MDS_DISKGROUP_DROP_FAILED
            self.response.rc.message = "Diskgroup drop failed: %s" % result
            self.SendResponse(self.response)
            return MS_FINISH
