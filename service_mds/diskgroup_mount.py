...
class DiskgroupMountMachine(BaseMachine):
    __metaclass__ = MataMachine

    MID = msg_mds.DISKGROUP_MOUNT_REQUEST

    def INIT(self, request):
        self.response     = MakeResponse(msg_mds.DISKGROUP_MOUNT_RESPONSE, request)
        self.request      = request
        self.request_body = request.body.Extensions[msg_mds.diskgroup_mount_request]
        ...
        if not diskgroup_info:
            ...
            self.SendResponse(self.response)
            return MS_FINISH
        if diskgroup_info.state == "MOUNTED":
            self.response.rc.retcode = msg_mds.RC_MDS_DISKGROUP_ALREADY_MOUNTED
            self.response.rc.message = "Diskgroup %s is already mounted" % diskgroup_name
            self.SendResponse(self.response)
            return MS_FINISH
        ...
        ret, result = sql.mount_asm_group(diskgroup_name)
        if ret:
            self.response.rc.retcode = msg_mds.RC_MDS_DISKGROUP_MOUNT_FAILED
            self.response.rc.message = "Mount asm diskgroup failed: %s" % result
            self.SendResponse(self.response)
            return MS_FINISH
