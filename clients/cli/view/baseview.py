...
class BaseView:
    ...
    # 字节单位转换(1000进制)
    def format_disk_size_1000(self, value="", params={}):
        size = ""
        if int(value) /1000/1000/1000 != 0:
            size = "%dG" % (int(value) /1000/1000/1000)
        elif int(value) /1000/1000 != 0:
            size = "%dM" % (int(value) /1000/1000)
        else:
            size = "0G"
    ...
    def format_dev_name(self, value="", params={}):
        if not value: value = "--"
        if params.has_key('cl') and params['cl'] == True:
            return value, False
        else:
    ...
    def format_unix_time(self, value="", params={}):
        if params.has_key('cl') and params['cl'] == True:
            return time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(value))), False
        else:
    ...
    def format_lun_dev_name(self, value="", params={}):
    ...
    def format_pool_dev_name(self, value="", params={}):
        if value != "--" and value.find("--") != -1:
            if params.has_key('cl') and params['cl'] == True:
                return value, True
            else:
    ...
    def format_pool_online_state(self, value="", params={}):
    ...
    def format_online_state(self, value="", params={}):
    ...
    def format_lun_state(self, value="", params={}):
        if value not in ["ONLINE", "OFFLINE", "ACTIVE", "INACTIVE", "SYNC", "DROPPING"]:
            if params.has_key('cl') and params['cl'] == True:
                return value, True
            else:
                return "\033[1;31m" + value + "\033[0m"
